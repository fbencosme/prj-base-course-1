...
class ExchangeFragment : BaseFragment() {
    ...
    fun onExchange(exchange: Pair<String, String>) {
        ...
        fromApi(Build.VERSION_CODES.N) {
            ...
            conversion1.text = Html.fromHtml(exchange.first, Html.FROM_HTML_MODE_LEGACY)

            @SuppressWarnings("deprecation")
            conversion2.text = Html.fromHtml(exchange.second, Html.FROM_HTML_MODE_LEGACY)
        }

        toApi(Build.VERSION_CODES.N, false) {
            conversion1.text = Html.fromHtml(exchange.first)
            conversion2.text = Html.fromHtml(exchange.second)
        }
